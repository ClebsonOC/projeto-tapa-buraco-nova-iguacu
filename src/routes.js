...
// Reativado: Importar 'admin' para usar FieldValue para unir arrays de fotos
const { firestore, admin } = require("./firebase-init");

const router = express.Router();
// Reativado: multer é necessário para a rota de adicionar fotos no relatório
...
// --- ROTA DE CRIAÇÃO COM OBSERVAÇÃO (sem alterações) ---
router.post("/salvar", async (req, res) => {
    try {
        const { rua, bairro, buracos, condicaoTempo, observacao, username } = req.body;
        ...
        for (const b of buracos) {
            const dadosParaFirestore = {
                ...
                fotosDriveLinks: [], // Inicia com um array de fotos vazio
        ...
        console.error("ERRO NO /api/salvar:", error);
        res.status(500).json({ error: error.message || "Erro interno ao salvar." });
    }
});

// --- ROTAS DE LEITURA, ATUALIZAÇÃO E DELEÇÃO (TAPA BURACOS) ---
...
// --- ROTAS DE EFETIVO (CORRIGIDO/READICIONADO) ---
